...
| `restart-server.ps1` keeps saying *"PID N still owns the port"* and gives up, with Last Run Result `0` | **An orphaned listener.** The `node` process outlived the `cmd.exe` the task launched, so it is no longer in the task's process tree. `Stop-ScheduledTask` and `schtasks /end` both report SUCCESS without touching it, and Task Scheduler goes on counting it as a running instance — so `Start-ScheduledTask` is a no-op that *also* reports success. The task's action never runs, which means `run-server.ps1` never gets to free the port from inside session 0 the way it normally does. Tell it apart by the log: `.trainbud\logs\server.log` has no line from any PID but the old one, and no `EADDRINUSE` | The server is **up** on the old build — nothing is broken, nothing was deployed. It is in session 0 under S4U, so ending it needs elevation. In an **admin** terminal: `Stop-Process -Id <N> -Force; Start-ScheduledTask -TaskName 'TrainBud Server'`, then re-run `restart-server.ps1` to confirm a *different* PID owns the port. The script prints this with the real PID filled in |
